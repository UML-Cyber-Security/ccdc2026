#!/bin/bash
# This script will install teleport, and install nginx if you want it too on RHEL-based distros


################################# IMPORTANT ###################################
# NOTE THIS SCRIPT IS ONLY FOR RHEL BASED DISTROS LIKE ROCKY OR CentOS
# DO NOT USE ON UBUNTU OR DEBIAN BASED MACHINE
################################# IMPORTANT ###################################


# Check if Podman is installed and install it if not
# For Rocky Linux 8 and similar RHEL 8-based distros
...
if command -v podman &> /dev/null; then
    ...
    # Install podman from the default repos (available in RHEL 8+ repos)
...
echo "Waiting for Teleport services to start (timeout: 90 seconds)..."
sleep 2
...
# If using NPM, ask if it's already installed
echo ""
read -p "Is Nginx Proxy Manager already installed? (y/n): " NPM_INSTALLED

if [[ "$NPM_INSTALLED" =~ ^[Yy]$ ]]; then
    # NPM already installed, just show configuration tutorial
    CURRENT_USER=$(whoami)
    cat << EOF
