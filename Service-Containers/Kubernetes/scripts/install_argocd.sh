...
cleanup_on_error() {
    echo ""
    echo "=========================================="
    echo "ERROR: EXITING IMMEDIATELY, DELETING ARGOCD & KILLING PORT FORWARD"
    echo "=========================================="
    # Kill port-forward if it is active
...
echo "==============================================================================="
echo "=== FORWARDING PORT 8080 FROM LOCAL MACHINE TO PORT 443 OF ARGOCD API SERVER ==="
echo "==============================================================================="
...
PORT_FORWARD_ARGOCD=$!
sleep 1
if ! ps -p $PORT_FORWARD_ARGOCD >/dev/null; then
    echo "Port-forward failed. Check /tmp/argocd-port-forward.log"
    cleanup_on_error
fi
# Unsure if I should remove "2>&1" to show error output 


echo ""
echo "================================================================"
echo "             TO STOP PORT FORWARDING LATER, RUN:"
echo "             kill $PORT_FORWARD_ARGOCD"
echo "================================================================"
echo ""
echo ""
echo "================================================================"
echo "===========ArgoCD accessible at http://localhost:8080==========="
